...
export function decodeRawMail(rawMail) {
  ...
  try {
    ...
    log_snippet("decoded", decoded);
    return decoded;
  } catch (err) {
    // Not valid UTF-8 (e.g. ISO-8859-1): keep the original byte values,
    // which already map 1:1 to the correct Latin-1 characters.
    console.warn(
      "decodeRawMail: not valid UTF-8 (" +
        err.message +
        "), falling back to Latin-1",
    );
    log_snippet("decodeRawMail (fallback)", rawMail, 120);
    return rawMail;
  }
}
 ...
 *
 * Tries the extended import_mail method first (available when
 * mail_manual_routing is installed), falling back to the standard
 * message_process on unmodified Odoo servers.
 *
 * import_mail returns a dict: {status, model, thread_id, message_id}
 * message_process returns: thread_id (int), false (duplicate), null (ignored)
 */
export async function uploadMail(cfg, rawMail, model = false) {
  ...
  const message = decodeRawMail(rawMail);

  // Try import_mail first (mail_manual_routing extension)
  try {
    console.debug(
      "uploadMail: sending message length=" +
        ...
        " to " +
        cfg.url +
        "/json/2/mail.thread/import_mail",
    );
    const result = await odooCall(cfg, "mail.thread/import_mail", {
      model: model,
      message: message,
    });
    console.debug("uploadMail: import_mail result=" + JSON.stringify(result));
    return result;
  } catch (err) {
    // import_mail not available — fall back to standard message_process
    console.debug(
      "uploadMail: import_mail not available (" +
        err.message +
        "), falling back to message_process",
    );
  }
